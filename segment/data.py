...
def parse_record(raw_record, config):
  ...
  image = tf.image.decode_image(tf.reshape(parsed['image/encoded'], shape=[]), config['input_shape'][2])
  image = tf.cast(tf.image.convert_image_dtype(image, dtype=tf.uint8), dtype=tf.float32)
  image.set_shape([None, None, config['input_shape'][2]])
...
def prepare_image(image, label, config):
    image = tf.cast(tf.image.resize_with_crop_or_pad(image, config['input_shape'][0], config['input_shape'][1]), dtype=tf.float32)
    label = tf.cast(tf.image.resize_with_crop_or_pad(label, config['input_shape'][0], config['input_shape'][1]), dtype=tf.float32)
...
def augment_image(image, label, config):
    ...
    shift_x = tf.random.uniform(shape=[], minval=-config['input_shape'][1]*config['augment_shift_x'],maxval=config['input_shape'][1]*config['augment_shift_x'],
        dtype=tf.dtypes.float32,name='scale_augmentation')
  
    shift_y = tf.random.uniform(shape=[], minval=-config['input_shape'][0]*config['augment_shift_y'],maxval=config['input_shape'][0]*config['augment_shift_y'],
        dtype=tf.dtypes.float32,name='scale_augmentation')
    ...
    m02 = tf.multiply(tf.math.subtract(1.0, alpha), config['input_shape'][1]/2.0)
    m02 = tf.math.subtract(m02, tf.multiply(beta,config['input_shape'][0]/2.0))
    m02 = tf.math.add(m02,shift_x)

    m12 = tf.multiply(tf.math.subtract(1.0, alpha),config['input_shape'][0]/2.0)
    m12 = tf.math.add( tf.multiply(beta, config['input_shape'][1]/2.0),m12)
