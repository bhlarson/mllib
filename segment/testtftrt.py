...
def main(args):
    ...
    onnxobjname = '{}/{}/{}'.format(s3def['sets']['model']['prefix'], config['initialmodel'], config['onnxmodel'])
    onnxfilename = '{}/{}/{}/{}'.format(args.test_dir, s3def['sets']['model']['prefix'], config['initialmodel'], config['onnxmodel'])
    ...
    with strategy.scope(): # Apply training strategy 
        ...
        try:
            ...
            for i in tqdm(range(numsteps)):
                ...
                logitstf = model.predict(image, batch_size=config['batch_size'], steps=1)
                segmentationtf = tf.argmax(logitstf, axis=-1)
                ...
                for j in range(config['batch_size']):
                    ...
                    tfseg = tf.squeeze(segmentationtf[j]).numpy().astype(np.uint8)
                    trtseg = tf.squeeze(segmentationtrt[j]).numpy().astype(np.uint8)
                    onnxseg = tf.squeeze(segmentationonnx[j]).numpy().astype(np.uint8)

                    font = cv2.FONT_HERSHEY_COMPLEX_SMALL
                    iman = DrawFeatures(img, ann, config)
                    iman = cv2.putText(iman, 'Annotation',(10,25), font, 1,(255,255,255),1,cv2.LINE_AA)
                    imtfseg = DrawFeatures(img, tfseg, config)
                    imtfseg = cv2.putText(imtfseg, 'Tensorflow',(10,25), font, 1,(255,255,255),1,cv2.LINE_AA)
                    imtrtseg = DrawFeatures(img, trtseg, config)
                    imtrtseg = cv2.putText(imtrtseg, 'TensorRT',(10,25), font, 1,(255,255,255),1,cv2.LINE_AA)
                    imonnxseg = DrawFeatures(img, onnxseg, config)
                    imonnxseg = cv2.putText(imtrtseg, 'ONNX',(10,25), font, 1,(255,255,255),1,cv2.LINE_AA)

                    im = cv2.hconcat([iman, imtfseg, imonnxseg, imtrtseg])
                    im_bgr = cv2.cvtColor(im, cv2.COLOR_RGB2BGR)
                    cv2.imwrite('{}/{}{:03d}{:03d}.png'.format(args.test_dir, 'seg', i, j), im_bgr)

                    accuracy.update_state(ann,tfseg)
                    seg_accuracy = accuracy.result().numpy()
                    tfaccuracySum += seg_accuracy
                    imagesimilarity, results['class similarity'], unique = jaccard(ann, tfseg, objTypes, results['class similarity'])

                    confusion = tf.math.confusion_matrix(ann.flatten(),tfseg.flatten(), config['classes']).numpy().astype(np.int64)
                    if total_confusion is None:
                        total_confusion = confusion
                    else:
                        total_confusion += confusion

                    accuracy.update_state(ann,trtseg)
                    seg_accuracy = accuracy.result().numpy()
                    trtaccuracySum += seg_accuracy
                    imagesimilarity, results['class similarity'], unique = jaccard(ann, trtseg, objTypes, results['class similarity'])

                    confusion = tf.math.confusion_matrix(ann.flatten(),trtseg.flatten(), config['classes']).numpy().astype(np.int64)
                    if total_confusion is None:
                        total_confusion = confusion
                    else:
                        total_confusion += confusion                  
