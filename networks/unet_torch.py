...
def parse_arguments():
    ...
    parser.add_argument('-model_src', type=str,  default='unet_512x442_20220215_03')
    parser.add_argument('-model_dest', type=str, default='unet_512x442_20220215_04')
    ...
    parser.add_argument('-class_weight', type=json.loads, default='[0.5,1.0, 1.0, 1.0]', help='Loss class weight ') 
...
def MakeNetwork(classes, args, s3, s3def):
    ...
    model = torch.hub.load('mateuszbuda/brain-segmentation-pytorch', 'unet', in_channels=3, out_channels=classes, init_features=64, pretrained=False)
...
def main(args):
    ...
    segment = MakeNetwork(class_dictionary['classes'], args, s3, s3def)
    ...
    compute_loss = nn.CrossEntropyLoss(weight=class_weight)
    ...
    if args.train:
        for epoch in tqdm(range(args.epochs), desc="Train epochs"):  # loop over the dataset multiple times
            ...
            for i, data in tqdm(enumerate(trainloader), total=trainingset.__len__()/args.batch_size, desc="Train steps"):
                ...
                loss = compute_loss(outputs, labels.long())
                loss.backward()
                optimizer.step()
                ...
                if i % test_freq == test_freq-1:    # Save image and run test
                    ...
                    for j in range(1):
                        ...
                        writer.add_image('segment/train', imanseg, 0,dataformats='HWC')
                    ...
                    msg = '[{:3}/{}, {:6d}/{}]  loss: {:0.5e}|{:0.5e} (train|test)'.format(
                        epoch + 1,args.epochs, i + 1, len(trainingset)/args.batch_size, running_loss, loss.item())
                    if args.job is True:
                        print(msg)
                    else:
                        tqdm.write(msg)
                    ...
                    for j in range(1):
                        ...
                        writer.add_image('segment/test', imanseg, 0,dataformats='HWC')
